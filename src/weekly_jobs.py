...
for column in columns:
    ...
    unique_values[f'{table.upper()}_{column.upper()}S'] = [row[0] for row in cur.fetchall() if row[0] is not None]
    if column == 'day_of_week':
        # Sort columns in weekday order
        unique_values[f'{table.upper()}_{column.upper()}S'] = sorted(unique_values[f'{table.upper()}_{column.upper()}S'], key=lambda x: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].index(x))
    elif column == 'month':
        # Sort columns in month order
        unique_values[f'{table.upper()}_{column.upper()}S'] = sorted(unique_values[f'{table.upper()}_{column.upper()}S'], key=lambda x: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'].index(x))
    elif column == 'spread':
        # Sort columns in the following order: exact values, "or more" values, "or less" values (by numeric value)
        contains_or_more = []
        contains_or_less = []
        for value in unique_values[f'{table.upper()}_{column.upper()}S']:
            if 'or more' in value:
                contains_or_more.append(value)
            elif 'or less' in value:
                contains_or_less.append(value)
        without_or_more_or_less = [value for value in unique_values[f'{table.upper()}_{column.upper()}S'] if 'or more' not in value and 'or less' not in value]
        ...
        unique_values[f'{table.upper()}_{column.upper()}S'] = without_or_more_or_less + contains_or_more + contains_or_less
    elif column == 'total':
        # Sort columns in the following order: exact values, "or more" values, "or less" values (by numeric value)
        contains_or_more = []
        contains_or_less = []
        ...
        contains_or_more.sort(key=lambda x: float(x.split(' ')[0]))
        contains_or_less.sort(key=lambda x: float(x.split(' ')[0]))
        unique_values[f'{table.upper()}_{column.upper()}S'] = contains_or_more + contains_or_less
