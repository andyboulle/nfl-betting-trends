...
def create_database(df):
    ...
    #     Create GameCompleted game from the row
    #     For every TrendCondition trendCondition in game.condition_combinations
    #         If the Trends database has a Trend trend with the same description as trendCondition
    #             Add wins/losses/ties to trend
    #         Else
    #             Create new Trend trend with correct wins/losses/ties
    #             Add trend to Trends database
    ...
    for index, row in df.head(25).iterrows():
        ...
        if index == 23:
            json_string = json.dumps(game, indent=4, cls=MyEncoder)
            with open('output.txt', 'w') as file:
                file.write(json_string)
        
        for description in game.condition_combinations:
            if description == '':
                var = None
            else:
                var = None
